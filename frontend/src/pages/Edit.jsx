  ...
  useEffect(() => {
    const getUserTests = async () => {  
        ...
        if (targetTest) {
            ...
            backgroundImage: targetTest.backgroundImage || "",
            results: targetTest.results || [],
            });

            // Set initial `newQuestion` if needed
            if (targetTest.questions.length > 0) {
            setNewQuestion({
                question: targetTest.questions[0]?.question || "",
                options: targetTest.questions[0]?.options || ["", ""],
                optionResults: targetTest.questions[0]?.optionResults || [0, 0],
            });
            }

            // Set initial `newResult` if needed
            if (targetTest.results.length > 0) {
            setNewResult({
                title: targetTest.results[0]?.title || "",
                description: targetTest.results[0]?.description || "",
            });
            }
        }
    };
